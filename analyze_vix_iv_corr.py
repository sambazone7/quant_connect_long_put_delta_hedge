...
with open("logs-qqq/parsed-march28-20days.csv") as f:
    for r in csv.DictReader(f):
        ...
        iven = r.get("iv_entry", "").strip().rstrip("%")
        ivex = r.get("iv_exit", "").strip().rstrip("%")
...
print(f"Total trades with valid VIX + IV entry/exit: {len(rows)}")
print()
...
print("=" * 60)
print("1. CORRELATION: %dVIX vs %dIV")
print("=" * 60)
...
for pct_dvix, pct_div, sim_f, *_ in rows:
    if pct_dvix <= 0 and pct_div <= 0:
        quads["VIX down + IV down"].append((pct_dvix, pct_div, sim_f))
    elif pct_dvix <= 0 and pct_div > 0:
        quads["VIX down + IV up"].append((pct_dvix, pct_div, sim_f))
    elif pct_dvix > 0 and pct_div <= 0:
        quads["VIX up   + IV down"].append((pct_dvix, pct_div, sim_f))
    else:
        quads["VIX up   + IV up"].append((pct_dvix, pct_div, sim_f))

print("=" * 60)
print("3. QUADRANT ANALYSIS")
print("=" * 60)
hdr = f"  {'Quadrant':<24} {'Trades':>6} {'%':>5} {'Avg %dVIX':>10} {'Avg %dIV':>10} {'Avg SimPnL':>12} {'Win%':>6}"
print(hdr)
print("  " + "-" * (len(hdr) - 2))
for name in ["VIX down + IV down", "VIX down + IV up", "VIX up   + IV down", "VIX up   + IV up"]:
    data = quads[name]
    if not data:
...
# Bucketed by VIX change magnitude
print("=" * 60)
print("4. BUCKETED BY VIX % CHANGE — IV response")
print("=" * 60)
vix_buckets = [
...
hdr2 = f"  {'VIX Bucket':<20} {'Trades':>6} {'Avg %dVIX':>10} {'Avg %dIV':>10} {'Med %dIV':>10} {'Avg SimPnL':>12} {'Win%':>6}"
print(hdr2)
print("  " + "-" * (len(hdr2) - 2))
...
# Per-ticker correlation (top 10 by trade count)
print("=" * 60)
