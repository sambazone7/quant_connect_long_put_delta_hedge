...
from cal_mixin import _check_orphaned_positions, _on_end_of_algorithm
# endregion
...
class EarningsCalendarPutMultiTicker(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        for ticker in tickers:
            ...
            self._ts[ticker] = {
                ...
                "short_spread_exit":  0,
                "call_spread_exit":   0,
                "call_symbol":        None,     # call at same strike/expiry as long put (for sim PnL)
                ...
                "skips_wide_spread":  0,    # bid-ask spread > SPREAD_CUTOFF
    ...
    def _enter_position(self, ticker, earnings_dt):
        ...
        # ── Bid-ask spread cutoff ─────────────────────────────────────────────
        long_spread_cost  = abs(long_put.BidPrice - long_put.AskPrice) * 100 * n_contracts
        short_spread_cost = abs(short_put.BidPrice - short_put.AskPrice) * 100 * n_contracts
        if SPREAD_CUTOFF > 0:
            if long_spread_cost > SPREAD_CUTOFF or short_spread_cost > SPREAD_CUTOFF:
                self._log(f"  [{ticker}] SKIP — spread too wide "
                         f"(long=${long_spread_cost:.0f}, short=${short_spread_cost:.0f}, "
                         f"cutoff=${SPREAD_CUTOFF})")
                ts["skips_wide_spread"] += 1
                return
        ...
        # Subscribe to the call at the same strike/expiry as the long put (for sim PnL at exit)
        call_symbol = Symbol.CreateOption(
            ts["stock_symbol"],
            long_put.Symbol.ID.Market,
    ...
    def _exit_position(self, ticker):
        ...
        # ── Sim PnL (option-only, parity-based, no stock hedge) ──────────
        # Uses OTM side of put-call pair to avoid wide ITM bid-ask spreads.
        # sim = (short_entry_fill + effective_exit - long_entry_fill) × n × 100
        s_price = self.Securities[ts["stock_symbol"]].Price
        strike  = self.Securities[ts["put_symbol"]].Symbol.ID.StrikePrice

        if s_price >= strike:
            effective_exit = _mid(self.Securities[ts["put_symbol"]].BidPrice,
                                  self.Securities[ts["put_symbol"]].AskPrice)
        else:
            call_sym = ts.get("call_symbol")
            if call_sym and self.Securities.ContainsKey(call_sym):
                effective_exit = _mid(self.Securities[call_sym].BidPrice,
                                      self.Securities[call_sym].AskPrice)
            else:
                effective_exit = 0.0

        if effective_exit > 0 and ts["put_entry_fill"] > 0:
            sim_pnl = (ts["short_put_entry_fill"] + effective_exit
                       - ts["put_entry_fill"]) * n_contracts * 100
        else:
            sim_pnl = 0.0
        ...
        self._log(f"  [{ticker}] EXIT: LongPnL=${long_pnl:+,.2f} "
                 ...
                 f"Total=${total_pnl:+,.2f} SimPnL=${sim_pnl:+,.2f} "
    ...
    _check_orphaned_positions = _check_orphaned_positions
    OnEndOfAlgorithm = _on_end_of_algorithm
