...
M              = 5.0    # (DYNAMIC_ENTRY only) enter when IV_benefit$ > M × |daily_theta$|
...
class EarningsLongPutDeltaNeutral(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self._baseline_iv   = None   # IV captured on day K (first scan day); fixed thereafter
    ...
    def _scan_entry(self, earnings_dt):
        """
        Called every EOD in the scan window [entry_day, exit_day).
        On day K: captures the baseline IV of the candidate put.
        On K+1 onwards: enters when cumulative IV expansion benefit exceeds
        M × |dollar theta|, i.e. iv_change × dollar_vega > M × |dollar_theta|.
        """
        ...
        n_c   = max(1, int(S / (put_mid * 100)))
        vega  = put.Greeks.Vega
        theta = put.Greeks.Theta        # negative (daily time decay)
        try:
            current_iv = put.ImpliedVolatility
        except Exception:
            current_iv = 0.0

        dollar_vega  = vega  * n_c * 100
        dollar_theta = theta * n_c * 100   # negative

        today = self.Time.date()

        # Reset baseline when moving to a new earnings event
        if self._scan_earnings != ed:
            self._baseline_iv   = None
            self._scan_earnings = ed

        if self._baseline_iv is None:
            self._baseline_iv = current_iv
            return

        iv_change          = current_iv - self._baseline_iv
        iv_benefit         = iv_change * 100 * dollar_vega
        daily_dollar_theta = dollar_theta / 365
        threshold          = M * abs(daily_dollar_theta)

        if iv_benefit > threshold:
